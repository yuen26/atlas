...
import org.atlas.framework.event.core.SagaMode;
...
public class OrderService {
    ...
    private Map<Integer, CustomerDto> fetchCustomers(List<Order> orders) {
        List<Integer> userIds = orders.stream()
            ...
            .distinct()
            .toList();
        List<CustomerDto> customers = customerServiceClient.listCustomer(userIds);
    ...
    public static OrderDto toOrderDto(
        ...
        // Customer
        CustomerDto customer = customerMap.get(order.getUserId());
        if (customer != null) {
